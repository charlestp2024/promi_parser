...
def handle_foreign_filing_countries(
    session: Session, row: dict, subdocket_id: int, user_id: str
):
    ...
    for name in countries_raw.split(","):
        ...
        logger.info(
            f"✓ Foreign country '{country_name}' linked to subdocket ID {subdocket_id}"
        )
...
def save_subdocket(
    ...
    handle_foreign_filing_countries(session, row, subdocket.id, import_user_id)
...
def convert_subdocket_to_dto(subdocket: Subdocket) -> SubdocketDTO:
    ...
    role_map = {
        ur.role.role: ur.user_id for ur in subdocket.subdocket_user_role_mappings
    }
    inventor_ids = [
        ur.user_id
    ...
    # 2. Process other relationships using list comprehensions
    assignee_ids = 1
    foreign_country_ids = [
        mapping.country_id
    ...
    return SubdocketDTO(
        ...
        # Related IDs
        analyst_id=role_map.get(RoleEnum.ANALYST.label),
