        ...
        if level == "subdocket level":
            try:
                save_subdocket(
                    session=csd_session,
                    row=row,
                    docket_uuid=ic_docket_dto.uuid,
                    docket_number=ic_docket_dto.system_generated_docket_number,
                    import_user=ic_docket_dto.import_user,
                )
                logger.info("✓ Subdocket saved for row %s", idx)
            except Exception as e:
                logger.error(
                    "❌ Subdocket save failed row %s: %s", idx, e, exc_info=True
                )
