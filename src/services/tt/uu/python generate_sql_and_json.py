...
INPUT_CLIENT_HIERARCHY_JSON = (
    "client_hierarchy.json"  # Corrected to match the generator's output
)
...
def main():
    ...
    for full_name, user_uuid in user_map.items():
        ...
        invd_sql.append(
            f"INSERT INTO `user` (`full_name`, `uuid`, `first_name`, `last_name`, `tenant_id`, `email`, `deleted`, `inactive`) VALUES ('{escape_sql(full_name)}', '{user_uuid}', '{escape_sql(first_name)}', '{escape_sql(last_name)}', '{TENANT_ID}', '{escape_sql(email)}', 0, 0);"
        )
        drafting_sql.append(
            f"INSERT INTO `user` (`id`, `fullname`, `tenant_id`, `email`, `contact`, `firstname`, `lastname`, `middlename`, `deleted`, `inactive`, `created_on`, `modified_on`) VALUES ('{user_uuid}', '{escape_sql(full_name)}', '{TENANT_ID}', '{email}', NULL, '{escape_sql(first_name)}', '{escape_sql(last_name)}', NULL, 0, 0, '{NOW_TIMESTAMP}', '{NOW_TIMESTAMP}');"
        )
        docket_review_sql.append(
            f"INSERT INTO `users` (`uuid`, `first_name`, `last_name`, `full_name`, `email`, `address`, `contact`, `inactive`, `tenant_id`, `created_on`, `modified_on`) VALUES ('{user_uuid}', '{escape_sql(first_name)}', '{escape_sql(last_name)}', '{escape_sql(full_name)}', '{email}', NULL, NULL, 0, '{TENANT_ID}', '{NOW_TIMESTAMP}', '{NOW_TIMESTAMP}');"
        )
    ...
    for client_name, data in client_hierarchy.items():
        ...
        # --- Client INSERTs ---
        invd_sql.append(
            f"INSERT INTO `client` (`uuid`, `tenant_id`, `client_name`, `client_code`, `is_active`) VALUES ('{client_uuid}', '{TENANT_ID}', '{escape_sql(client_name)}', {client_code_counter}, 1);"
        )
        drafting_sql.append(
            f"INSERT INTO `client` (`id`, `client_code`, `client_name`, `email`, `is_active`, `created_on`, `modified_on`, `tenant_id`) VALUES ('{client_uuid}', {client_code_counter}, '{escape_sql(client_name)}', '{client_email}', 1, '{NOW_TIMESTAMP}', '{NOW_TIMESTAMP}', '{TENANT_ID}');"
        )
        docket_review_sql.append(
            f"INSERT INTO `client` (`uuid`, `client_name`, `client_code`, `is_active`, `tenant_id`, `created_on`, `modified_on`) VALUES ('{client_uuid}', '{escape_sql(client_name)}', {client_code_counter}, 1, '{TENANT_ID}', '{NOW_TIMESTAMP}', '{NOW_TIMESTAMP}');"
        )
        ...
        for dept_name, dept_uuid in data["departments"].items():
            invd_sql.append(
                f"INSERT INTO `department` (`uuid`, `tenant_id`, `client_id`, `department`) VALUES ('{dept_uuid}', '{TENANT_ID}', '{client_uuid}', '{escape_sql(dept_name)}');"
            )
            drafting_sql.append(
                f"INSERT INTO `department` (`uuid`, `department`, `client_id`, `tenant_id`, `created_on`, `modified_on`) VALUES ('{dept_uuid}', '{escape_sql(dept_name)}', '{client_uuid}', '{TENANT_ID}', '{NOW_TIMESTAMP}', '{NOW_TIMESTAMP}');"
            )
            docket_review_sql.append(
        ...
        for div_name, div_uuid in data["divisions"].items():
            invd_sql.append(
                f"INSERT INTO `client_division` (`id`, `division_name`, `tenant_id`, `client_id`, `is_active`) VALUES ('{div_uuid}', '{escape_sql(div_name)}', '{TENANT_ID}', '{client_uuid}', 1);"
            )
            drafting_sql.append(
                f"INSERT INTO `client_division` (`id`, `division_name`, `client_id`, `is_active`, `created_on`, `modified_on`, `tenant_id`) VALUES ('{div_uuid}', '{escape_sql(div_name)}', '{client_uuid}', 1, '{NOW_TIMESTAMP}', '{NOW_TIMESTAMP}', '{TENANT_ID}');"
            )
